...
# 计算密度
set total_instances [$block getInsts]
set placed_instances 0
set total_inst_count 0
foreach inst $total_instances {{
    if {{[$inst isPlaced]}} {{
        incr placed_instances
    }}
    incr total_inst_count
}}
set density [expr double($placed_instances) / $total_inst_count]
puts "布局密度: $density"

# 计算面积（使用核心区域）
set core_area [expr 1390 * 1390]  # 核心区域面积（平方微米）
puts "核心面积: $core_area um²"

# 计算线长（使用估计值）
set estimated_wirelength [expr $core_area * 0.1]  # 估计线长为面积的10%
puts "估计线长: $estimated_wirelength"
