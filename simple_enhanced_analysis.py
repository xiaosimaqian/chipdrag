...
class SimpleEnhancedAnalysis:
    """简化的增强实验分析系统"""
    
    def __init__(self):
        self.base_dir = Path(__file__).parent
        self.successful_results_dir = self.base_dir / "results/ispd_training_fixed_v10"
        self.analysis_dir = self.base_dir / "simple_enhanced_analysis"
    ...
    def generate_visualizations(self, experiment_data: Dict[str, Any], 
        ...
        # 4. 基准测试性能对比图
        self._plot_benchmark_performance(experiment_data, viz_dir)
    ...
    def _plot_execution_time_distribution(self, experiment_data: Dict[str, Any], viz_dir: Path):
        ...
        # 直方图
        ax1.hist(execution_times, bins=8, alpha=0.7, color='skyblue', edgecolor='black')
    ...
    def _plot_benchmark_performance(self, experiment_data: Dict[str, Any], viz_dir: Path):
        """绘制基准测试性能对比图"""
        benchmarks = list(experiment_data.keys())
        execution_times = [experiment_data[b]['execution_time'] for b in benchmarks]
        categories = [self._categorize_design(b) for b in benchmarks]
        ...
        plt.title('各基准测试执行时间对比', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(viz_dir / 'benchmark_performance.png', dpi=300, bbox_inches='tight')
        plt.close()
...
"""
    ...
    def save_analysis_results(self, experiment_data: Dict[str, Any], 
        ...
        analysis_results = {
            ...
            'metadata': {
                ...
                'version': 'v1.0',
                'description': '简化的增强实验分析结果'
            }
        }
        
        results_file = self.analysis_dir / "simple_enhanced_analysis_results.json"
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump(analysis_results, f, indent=2, ensure_ascii=False)
        
        # 保存综合报告
        report_file = self.analysis_dir / "simple_comprehensive_analysis_report.md"
    ...
    def run_enhanced_analysis(self) -> Dict[str, Any]:
        ...
        # 3. 生成可视化
        self.generate_visualizations(experiment_data, statistical_analysis)
...
def main():
    """主函数"""
    logger.info("启动简化增强实验分析系统...")
    
    # 创建分析系统
    analysis_system = SimpleEnhancedAnalysis()
